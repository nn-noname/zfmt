...
const BraceStyle = enum {
    k_and_r, // fn main() {
    allman, // fn main()\n{
    disabled, // no brace formatting
};

pub const Options = struct {
    indent_delta: usize = 4,
    asm_indent_delta: usize = 4,
...
pub fn renderTree(gpa: Allocator, w: *Writer, tree: Ast, fixups: Fixups, options: Options) Error!void {
    assert(tree.errors.len == 0); // Cannot render an invalid tree.
    var auto_indenting_stream: AutoIndentingStream = .init(gpa, w, options.indent_delta);
    ...
    _ = try renderComments(&r, 0, comment_end_loc, options);

    if (tree.tokenTag(0) == .container_doc_comment) {
        try renderContainerDocComments(&r, 0, options);
    }

    switch (tree.mode) {
        .zig => try renderMembers(&r, tree.rootDecls(), options),
        .zon => {
    ...
    if (auto_indenting_stream.disabled_offset) |disabled_offset| {
        try writeFixingWhitespace(auto_indenting_stream.underlying_writer, tree.source[disabled_offset..], options);
    }
}

/// Render all members in the given slice, keeping empty lines where appropriate
fn renderMembers(r: *Render, members: []const Ast.Node.Index, options: Options) Error!void {
    const tree = r.tree;
    ...
    try renderMember(r, container, members[0], .newline, options);
    for (members[1..]) |member| {
        try renderExtraNewline(r, member, options);
        try renderMember(r, container, member, .newline, options);
    }
}
...
/// Render a single member declaration within a container, handling various node types
/// including function declarations, variable declarations, test declarations, and container fields.
fn renderMember(
    r: *Render,
    container: Container,
    decl: Ast.Node.Index,
    ...
    try renderDocComments(r, tree.firstToken(decl), options);
    switch (tree.nodeTag(decl)) {
        .fn_decl => {
            ...
            while (i < fn_token) : (i += 1) {
                try renderToken(r, i, .space, options);
            }
            ...
            try renderExpression(r, fn_proto, .space, options);
            if (r.fixups.gut_functions.contains(decl)) {
                try ais.pushIndent(.normal);
                const lbrace = tree.nodeMainToken(body_node);
                try renderToken(r, lbrace, .newline, options);
                try discardAllParams(r, fn_proto, options);
                try ais.writeAll("@trap();");
                ais.popIndent();
                try ais.insertNewline();
                try renderToken(r, tree.lastToken(body_node), space, options); // rbrace
                ...
                while (it.next()) |param| {
                    ...
                    if (r.fixups.unused_var_decls.contains(name_ident)) {
                        try ais.writeAll("_ = ");
                        try ais.writeAll(tokenSliceForRender(r.tree, name_ident, options));
                        try ais.writeAll(";\n");
                    }
                }
                var statements_buf: [2]Ast.Node.Index = undefined;
                const statements = tree.blockStatements(&statements_buf, body_node).?;
                return finishRenderBlock(r, body_node, statements, space, options);
            } else {
                return renderExpression(r, body_node, space, options);
            }
        },
        .fn_proto_simple,
        .fn_proto_multi,
        ...
        => {
            ...
            try renderExpression(r, decl, .none, options);
            return renderToken(r, tree.lastToken(decl) + 1, space, options); // semicolon
        },
        ...
        => {
            ...
            try renderVarDecl(r, tree.fullVarDecl(decl).?, false, .semicolon, options);
            ais.popSpace();
        },
        ...
        .test_decl => {
            ...
            try renderToken(r, test_token, .space, options);
            if (opt_name_token.unwrap()) |name_token| {
                switch (tree.tokenTag(name_token)) {
                    .string_literal => try renderToken(r, name_token, .space, options),
                    .identifier => try renderIdentifier(r, name_token, .space, .preserve_when_shadowing, options),
                    else => unreachable,
                }
            }
            try renderExpression(r, block_node, space, options);
        },

        .container_field_init,
        .container_field_align,
        .container_field,
        => return renderContainerField(r, container, tree.fullContainerField(decl).?, space, options),

        .@"comptime" => return renderExpression(r, decl, space, options),
...
fn renderExpressions(r: *Render, expressions: []const Ast.Node.Index, space: Space, options: Options) Error!void {
    if (expressions.len == 0) return;
    try renderExpression(r, expressions[0], space, options);
    for (expressions[1..]) |expression| {
        try renderExtraNewline(r, expression, options);
        try renderExpression(r, expression, space, options);
    }
}

/// Render any AST expression node, handling all expression types including literals,
/// operators, function calls, control flow, and complex expressions.
fn renderExpression(r: *Render, node: Ast.Node.Index, space: Space, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    if (r.fixups.replace_nodes_with_string.get(node)) |replacement| {
        ...
        return renderExpression(r, replacement, space, options);
    }
    switch (tree.nodeTag(node)) {
        .identifier => {
            const token_index = tree.nodeMainToken(node);
            return renderIdentifier(r, token_index, space, .preserve_when_shadowing, options);
        },
        ...
        .anyframe_literal,
        .string_literal,
        => return renderToken(r, tree.nodeMainToken(node), space, options),
        ...
        .multiline_string_literal => {
            ...
            for (first_tok..last_tok + 1) |i| {
                try renderToken(r, @intCast(i), .newline, options);
            }
            ...
            switch (space) {
                ...
                .semicolon => if (next_token_tag == .semicolon) try renderTokenOverrideSpaceMode(r, next_token, .newline, .semicolon, options),
                .comma => if (next_token_tag == .comma) try renderTokenOverrideSpaceMode(r, next_token, .newline, .comma, options),
                .comma_space => if (next_token_tag == .comma) try renderToken(r, next_token, .space, options),
            }
        },

        .error_value => {
            const main_token = tree.nodeMainToken(node);
            try renderToken(r, main_token, .none, options);
            try renderToken(r, main_token + 1, .none, options);
            return renderIdentifier(r, main_token + 2, space, .eagerly_unquote, options);
        },
        ...
        => {
            ...
            return renderBlock(r, node, statements, space, options);
        },
        ...
        .@"errdefer" => {
            ...
            try renderToken(r, defer_token, .space, options);
            if (maybe_payload_token.unwrap()) |payload_token| {
                try renderToken(r, payload_token - 1, .none, options); // |
                try renderIdentifier(r, payload_token, .none, .preserve_when_shadowing, options); // identifier
                try renderToken(r, payload_token + 1, .space, options); // |
            }
            return renderExpression(r, expr, space, options);
        },
        ...
        => {
            ...
            try renderToken(r, main_token, .space, options);
            return renderExpression(r, item, space, options);
        },
        ...
        .@"catch" => {
            ...
            try renderExpression(r, lhs, .space, options); // target

            try ais.pushIndent(.normal);
            if (tree.tokenTag(fallback_first - 1) == .pipe) {
                try renderToken(r, main_token, .space, options); // catch keyword
                try renderToken(r, main_token + 1, .none, options); // pipe
                try renderIdentifier(r, main_token + 2, .none, .preserve_when_shadowing, options); // payload identifier
                try renderToken(r, main_token + 3, after_op_space, options); // pipe
            } else {
                assert(tree.tokenTag(fallback_first - 1) == .keyword_catch);
                try renderToken(r, main_token, after_op_space, options); // catch keyword
            }
            try renderExpression(r, rhs, space, options); // fallback
            ais.popIndent();
        },
        ...
        .field_access => {
            ...
            try renderExpression(r, lhs, .none, options);
            ...
            try renderToken(r, dot_token, .none, options);

            try renderIdentifier(r, name_token, space, .eagerly_unquote, options); // field
            ais.popIndent();
        },
        ...
        => {
            const lhs, const rhs = tree.nodeData(node).node_and_node;
            try renderExpression(r, lhs, .none, options);
            try renderToken(r, tree.nodeMainToken(node), .none, options);
            return renderExpression(r, rhs, space, options);
        },
        .for_range => {
            const start, const opt_end = tree.nodeData(node).node_and_opt_node;
            try renderExpression(r, start, .none, options);
            if (opt_end.unwrap()) |end| {
                try renderToken(r, tree.nodeMainToken(node), .none, options);
                return renderExpression(r, end, space, options);
            } else {
                return renderToken(r, tree.nodeMainToken(node), space, options);
            }
        },
        ...
        => {
            const lhs, const rhs = tree.nodeData(node).node_and_node;
            try renderExpression(r, lhs, .space, options);
            const op_token = tree.nodeMainToken(node);
            try ais.pushIndent(.after_equals);
            if (tree.tokensOnSameLine(op_token, op_token + 1)) {
                try renderToken(r, op_token, .space, options);
            } else {
                try renderToken(r, op_token, .newline, options);
            }
            try renderExpression(r, rhs, space, options);
            ais.popIndent();
        },
        ...
        => {
            ...
            const space_or_none = if (options.space_around_ops == true) Space.space else Space.none;

            try renderExpression(r, lhs, space_or_none, options);
            const op_token = tree.nodeMainToken(node);
            try ais.pushIndent(.binop);
            if (tree.tokensOnSameLine(op_token, op_token + 1)) {
                try renderToken(r, op_token, space_or_none, options);
            } else {
                try renderToken(r, op_token, .newline, options);
            }
            try renderExpression(r, rhs, space, options);
        ...
        .assign_destructure => {
            ...
            if (full.comptime_token) |comptime_token| {
                try renderToken(r, comptime_token, .space, options);
            }
            ...
            for (full.ast.variables, 0..) |variable_node, i| {
                ...
                switch (tree.nodeTag(variable_node)) {
                    ...
                    .aligned_var_decl,
                    => {
                        try renderVarDecl(r, tree.fullVarDecl(variable_node).?, true, variable_space, options);
                    },
                    else => try renderExpression(r, variable_node, variable_space, options),
                }
            }
            try ais.pushIndent(.after_equals);
            if (tree.tokensOnSameLine(full.ast.equal_token, full.ast.equal_token + 1)) {
                try renderToken(r, full.ast.equal_token, .space, options);
            } else {
                try renderToken(r, full.ast.equal_token, .newline, options);
            }
            try renderExpression(r, full.ast.value_expr, space, options);
            ais.popIndent();
        },
        ...
        => {
            try renderToken(r, tree.nodeMainToken(node), .none, options);
            return renderExpression(r, tree.nodeData(node).node, space, options);
        },

        .@"try",
        .@"resume",
        => {
            try renderToken(r, tree.nodeMainToken(node), .space, options);
            return renderExpression(r, tree.nodeData(node).node, space, options);
        },

        .array_type,
        .array_type_sentinel,
        => return renderArrayType(r, tree.fullArrayType(node).?, space, options),

        .ptr_type_aligned,
        .ptr_type_sentinel,
        .ptr_type,
        .ptr_type_bit_range,
        => return renderPtrType(r, tree.fullPtrType(node).?, space, options),
        ...
        => {
            ...
            return renderArrayInit(r, tree.fullArrayInit(&elements, node).?, space, options);
        },
        ...
        => {
            var buf: [2]Ast.Node.Index = undefined;
            return renderStructInit(r, node, tree.fullStructInit(&buf, node).?, space, options);
        },
        ...
        => {
            var buf: [1]Ast.Node.Index = undefined;
            return renderCall(r, tree.fullCall(&buf, node).?, space, options);
        },
        ...
        .array_access => {
            ...
            try renderExpression(r, lhs, .none, options);
            try ais.pushIndent(.normal);
            try renderToken(r, lbracket, inner_space, options); // [
            try renderExpression(r, rhs, inner_space, options);
            ais.popIndent();
            return renderToken(r, rbracket, space, options); // ]
        },

        .slice_open,
        .slice,
        .slice_sentinel,
        => return renderSlice(r, node, tree.fullSlice(node).?, space, options),

        .deref => {
            try renderExpression(r, tree.nodeData(node).node, .none, options);
            return renderToken(r, tree.nodeMainToken(node), space, options);
        },
        ...
        .unwrap_optional => {
            ...
            try renderExpression(r, lhs, .none, options);
            try renderToken(r, dot_token, .none, options);
            return renderToken(r, question_mark, space, options);
        },
        ...
        .@"break", .@"continue" => {
            ...
            if (opt_label_token == .none and opt_target == .none) {
                try renderToken(r, main_token, space, options); // break/continue
            } else if (opt_label_token == .none and opt_target != .none) {
                const target = opt_target.unwrap().?;
                try renderToken(r, main_token, .space, options); // break/continue
                try renderExpression(r, target, space, options);
            } else if (opt_label_token != .none and opt_target == .none) {
                const label_token = opt_label_token.unwrap().?;
                try renderToken(r, main_token, .space, options); // break/continue
                try renderToken(r, label_token - 1, .none, options); // :
                try renderIdentifier(r, label_token, space, .eagerly_unquote, options); // identifier
            } else if (opt_label_token != .none and opt_target != .none) {
                const label_token = opt_label_token.unwrap().?;
                const target = opt_target.unwrap().?;
                try renderToken(r, main_token, .space, options); // break/continue
        ...
        .@"return" => {
            if (tree.nodeData(node).opt_node.unwrap()) |expr| {
                try renderToken(r, tree.nodeMainToken(node), .space, options);
                try renderExpression(r, expr, space, options);
            } else {
                try renderToken(r, tree.nodeMainToken(node), space, options);
            }
        },
        ...
        .grouped_expression => {
            ...
            try ais.pushIndent(.normal);
            try renderToken(r, tree.nodeMainToken(node), .none, options); // lparen
            try renderExpression(r, expr, .none, options);
            ais.popIndent();
            return renderToken(r, rparen, space, options);
        },
        ...
        => {
            var buf: [2]Ast.Node.Index = undefined;
            return renderContainerDecl(r, node, tree.fullContainerDecl(&buf, node).?, space, options);
        },
        ...
        .error_set_decl => {
            ...
            try renderToken(r, error_token, .none, options);

            if (lbrace + 1 == rbrace) {
                // There is nothing between the braces so render condensed: `error{}`
                try renderToken(r, lbrace, .none, options);
                return renderToken(r, rbrace, space, options);
                ...
                try renderToken(r, lbrace, .none, options);
                try renderIdentifier(r, lbrace + 1, .none, .eagerly_unquote, options); // identifier
                return renderToken(r, rbrace, space, options);
                ...
                var i = lbrace + 1;
                while (i < rbrace) : (i += 1) {
                    if (i > lbrace + 1) try renderExtraNewlineToken(r, i, options);
                    switch (tree.tokenTag(i)) {
                        .doc_comment => try renderToken(r, i, .newline, options),
                        .identifier => {
                            try ais.pushSpace(.comma);
                            try renderIdentifier(r, i, .comma, .eagerly_unquote, options);
                            ais.popSpace();
                        },
                        .comma => {},
                        else => unreachable,
                ...
                try renderToken(r, lbrace, .space, options);
                var i = lbrace + 1;
                while (i < rbrace) : (i += 1) {
                    switch (tree.tokenTag(i)) {
                        .doc_comment => unreachable, // TODO
                        .identifier => try renderIdentifier(r, i, .comma_space, .eagerly_unquote, options),
                        .comma => {},
                        else => unreachable,
                    }
                }
        ...
        => {
            ...
            return renderBuiltinCall(r, builtin_token, params, space, options);
        },
        ...
        => {
            var buf: [1]Ast.Node.Index = undefined;
            return renderFnProto(r, tree.fullFnProto(&buf, node).?, space, options);
        },

        .anyframe_type => {
            const main_token = tree.nodeMainToken(node);
            try renderToken(r, main_token, .none, options); // anyframe
            try renderToken(r, main_token + 1, .none, options); // ->
            return renderExpression(r, tree.nodeData(node).token_and_node[1], space, options);
        },
        ...
        => {
            ...
            if (full.label_token) |label_token| {
                try renderIdentifier(r, label_token, .none, .eagerly_unquote, options); // label
                try renderToken(r, label_token + 1, .space, options); // :
            }

            const rparen = tree.lastToken(full.ast.condition) + 1;

            try renderToken(r, full.ast.switch_token, .space, options); // switch
            try renderToken(r, full.ast.switch_token + 1, .none, options); // (
            try renderExpression(r, full.ast.condition, .none, options); // condition expression
            try renderToken(r, rparen, .space, options); // )

            try ais.pushIndent(.normal);
            if (full.ast.cases.len == 0) {
                try renderToken(r, rparen + 1, .none, options); // {
            } else {
                try renderToken(r, rparen + 1, .newline, options); // {
                try ais.pushSpace(.comma);
                try renderExpressions(r, full.ast.cases, .comma, options);
                ais.popSpace();
            }
            ais.popIndent();
            return renderToken(r, tree.lastToken(node), space, options); // }
        },
        ...
        .switch_case,
        .switch_case_inline,
        => return renderSwitchCase(r, tree.fullSwitchCase(node).?, space, options),

        .while_simple,
        .while_cont,
        .@"while",
        => return renderWhile(r, tree.fullWhile(node).?, space, options),

        .for_simple,
        .@"for",
        => return renderFor(r, tree.fullFor(node).?, space, options),

        .if_simple,
        .@"if",
        => return renderIf(r, tree.fullIf(node).?, space, options),

        .asm_simple,
        .@"asm",
        => return renderAsm(r, tree.fullAsm(node).?, space, options),

        // To be removed after 0.15.0 is tagged
        .asm_legacy => return renderAsmLegacy(r, tree.legacyAsm(node).?, space, options),

        .enum_literal => {
            try renderToken(r, tree.nodeMainToken(node) - 1, .none, options); // .
            return renderIdentifier(r, tree.nodeMainToken(node), space, .eagerly_unquote, options); // name
        },
...
fn renderExpressionFixup(
    ...
    const ais = r.ais;
    try renderExpression(r, node, space, options);
    if (r.fixups.append_string_after_node.get(node)) |bytes| {
        try ais.writeAll(bytes);
    }
}

/// Render an array type expression, handling both regular arrays and sentinel-terminated arrays.
fn renderArrayType(
    r: *Render,
    ...
    const inner_space = if (one_line) Space.none else Space.newline;
    try ais.pushIndent(.normal);
    try renderToken(r, array_type.ast.lbracket, inner_space, options); // lbracket
    try renderExpression(r, array_type.ast.elem_count, inner_space, options);
    if (array_type.ast.sentinel.unwrap()) |sentinel| {
        try renderToken(r, tree.firstToken(sentinel) - 1, inner_space, options); // colon
        try renderExpression(r, sentinel, inner_space, options);
    }
    ais.popIndent();
    try renderToken(r, rbracket, .none, options); // rbracket
    return renderExpression(r, array_type.ast.elem_type, space, options);
}

/// Render a pointer type expression, handling all pointer types including single, many, C, and slice pointers
/// with optional alignment, address space, and sentinel specifications.
fn renderPtrType(r: *Render, ptr_type: Ast.full.PtrType, space: Space, options: Options) Error!void {
    const tree = r.tree;
    ...
    switch (ptr_type.size) {
        .one => {
            ...
            {
                return renderExpression(r, ptr_type.ast.child_type, space, options);
            }
            try renderToken(r, main_token, .none, options); // asterisk
        },
        .many => {
            if (ptr_type.ast.sentinel.unwrap()) |sentinel| {
                try renderToken(r, main_token, .none, options); // lbracket
                try renderToken(r, main_token + 1, .none, options); // asterisk
                try renderToken(r, main_token + 2, .none, options); // colon
                try renderExpression(r, sentinel, .none, options);
                try renderToken(r, tree.lastToken(sentinel) + 1, .none, options); // rbracket
            } else {
                try renderToken(r, main_token, .none, options); // lbracket
                try renderToken(r, main_token + 1, .none, options); // asterisk
                try renderToken(r, main_token + 2, .none, options); // rbracket
            }
        },
        .c => {
            try renderToken(r, main_token, .none, options); // lbracket
            try renderToken(r, main_token + 1, .none, options); // asterisk
            try renderToken(r, main_token + 2, .none, options); // c
            try renderToken(r, main_token + 3, .none, options); // rbracket
        },
        .slice => {
            if (ptr_type.ast.sentinel.unwrap()) |sentinel| {
                try renderToken(r, main_token, .none, options); // lbracket
                try renderToken(r, main_token + 1, .none, options); // colon
                try renderExpression(r, sentinel, .none, options);
                try renderToken(r, tree.lastToken(sentinel) + 1, .none, options); // rbracket
            } else {
                try renderToken(r, main_token, .none, options); // lbracket
                try renderToken(r, main_token + 1, .none, options); // rbracket
            }
        },
    }

    if (ptr_type.allowzero_token) |allowzero_token| {
        try renderToken(r, allowzero_token, .space, options);
    }

    if (ptr_type.ast.align_node.unwrap()) |align_node| {
        const align_first = tree.firstToken(align_node);
        try renderToken(r, align_first - 2, .none, options); // align
        try renderToken(r, align_first - 1, .none, options); // lparen
        try renderExpression(r, align_node, .none, options);
        if (ptr_type.ast.bit_range_start.unwrap()) |bit_range_start| {
            const bit_range_end = ptr_type.ast.bit_range_end.unwrap().?;
            try renderToken(r, tree.firstToken(bit_range_start) - 1, .none, options); // colon
            try renderExpression(r, bit_range_start, .none, options);
            try renderToken(r, tree.firstToken(bit_range_end) - 1, .none, options); // colon
            try renderExpression(r, bit_range_end, .none, options);
            try renderToken(r, tree.lastToken(bit_range_end) + 1, .space, options); // rparen
        } else {
            try renderToken(r, tree.lastToken(align_node) + 1, .space, options); // rparen
        }
    }

    if (ptr_type.ast.addrspace_node.unwrap()) |addrspace_node| {
        const addrspace_first = tree.firstToken(addrspace_node);
        try renderToken(r, addrspace_first - 2, .none, options); // addrspace
        try renderToken(r, addrspace_first - 1, .none, options); // lparen
        try renderExpression(r, addrspace_node, .none, options);
        try renderToken(r, tree.lastToken(addrspace_node) + 1, .space, options); // rparen
    }

    if (ptr_type.const_token) |const_token| {
        try renderToken(r, const_token, .space, options);
    }

    if (ptr_type.volatile_token) |volatile_token| {
        try renderToken(r, volatile_token, .space, options);
    }

    try renderExpression(r, ptr_type.ast.child_type, space, options);
}

/// Render a slice expression with optional start, end, and sentinel values.
fn renderSlice(
    r: *Render,
    ...
    try renderExpression(r, slice.ast.sliced, .none, options);
    try renderToken(r, slice.ast.lbracket, .none, options); // lbracket

    const start_last = tree.lastToken(slice.ast.start);
    try renderExpression(r, slice.ast.start, after_start_space, options);
    try renderToken(r, start_last + 1, after_dots_space, options); // ellipsis2 ("..")

    if (slice.ast.end.unwrap()) |end| {
        const after_end_space = if (slice.ast.sentinel != .none) Space.space else Space.none;
        try renderExpression(r, end, after_end_space, options);
    }

    if (slice.ast.sentinel.unwrap()) |sentinel| {
        try renderToken(r, tree.firstToken(sentinel) - 1, .none, options); // colon
        try renderExpression(r, sentinel, .none, options);
    }

    try renderToken(r, tree.lastToken(slice_node), space, options); // rbracket
}

/// Render an assembly output constraint specification.
fn renderAsmOutput(
    r: *Render,
    ...
    try renderToken(r, symbolic_name - 1, .none, options); // lbracket
    try renderIdentifier(r, symbolic_name, .none, .eagerly_unquote, options); // ident
    try renderToken(r, symbolic_name + 1, .space, options); // rbracket
    try renderToken(r, symbolic_name + 2, .space, options); // "constraint"
    try renderToken(r, symbolic_name + 3, .none, options); // lparen

    if (tree.tokenTag(symbolic_name + 4) == .arrow) {
        const type_expr, const rparen = tree.nodeData(asm_output).opt_node_and_token;
        try renderToken(r, symbolic_name + 4, .space, options); // ->
        try renderExpression(r, type_expr.unwrap().?, Space.none, options);
        return renderToken(r, rparen, space, options);
    } else {
        try renderIdentifier(r, symbolic_name + 4, .none, .eagerly_unquote, options); // ident
        return renderToken(r, symbolic_name + 5, space, options); // rparen
    }
}

/// Render an assembly input constraint specification.
fn renderAsmInput(
    r: *Render,
    ...
    try renderToken(r, symbolic_name + 2, .space, options); // "constraint"
    try renderToken(r, symbolic_name + 3, .none, options); // lparen
    try renderExpression(r, expr, Space.none, options);
    return renderToken(r, rparen, space, options);
}

/// Render a variable declaration with optional fixups for unused variables.
fn renderVarDecl(
    r: *Render,
    ...
    try renderVarDeclWithoutFixups(r, var_decl, ignore_comptime_token, space, options);
    if (r.fixups.unused_var_decls.contains(var_decl.ast.mut_token + 1)) {
        // Discard the variable like this: `_ = foo;`
        const ais = r.ais;
        try ais.writeAll("_ = ");
        try ais.writeAll(tokenSliceForRender(r.tree, var_decl.ast.mut_token + 1, options));
        try ais.writeAll(";\n");
    }
}

/// Render a variable declaration without applying any fixups.
fn renderVarDeclWithoutFixups(
    ...
    if (var_decl.visib_token) |visib_token| {
        try renderToken(r, visib_token, Space.space, options); // pub
    }

    if (var_decl.extern_export_token) |extern_export_token| {
        try renderToken(r, extern_export_token, Space.space, options); // extern

        if (var_decl.lib_name) |lib_name| {
            try renderToken(r, lib_name, Space.space, options); // "lib"
        }
    }

    if (var_decl.threadlocal_token) |thread_local_token| {
        try renderToken(r, thread_local_token, Space.space, options); // threadlocal
    }

    if (!ignore_comptime_token) {
        if (var_decl.comptime_token) |comptime_token| {
            try renderToken(r, comptime_token, Space.space, options); // comptime
        }
    }

    try renderToken(r, var_decl.ast.mut_token, .space, options); // var
        ...
        var_decl.ast.init_node != .none)
    {
        ...
        try renderIdentifier(r, var_decl.ast.mut_token + 1, name_space, .preserve_when_shadowing, options); // name
    } else {
        return renderIdentifier(r, var_decl.ast.mut_token + 1, space, .preserve_when_shadowing, options); // name
    }

    if (var_decl.ast.type_node.unwrap()) |type_node| {
        try renderToken(r, var_decl.ast.mut_token + 2, Space.space, options); // :
        ...
        {
            try renderExpression(r, type_node, .space, options);
        } else {
            return renderExpression(r, type_node, space, options);
        }
    }
    ...
    if (var_decl.ast.align_node.unwrap()) |align_node| {
        ...
        try renderToken(r, align_kw, Space.none, options); // align
        try renderToken(r, lparen, Space.none, options); // (
        try renderExpression(r, align_node, Space.none, options);
    ...
    if (var_decl.ast.addrspace_node.unwrap()) |addrspace_node| {
        ...
        try renderToken(r, addrspace_kw, Space.none, options); // addrspace
        try renderToken(r, lparen, Space.none, options); // (
        try renderExpression(r, addrspace_node, Space.none, options);
        if (var_decl.ast.section_node != .none or var_decl.ast.init_node != .none) {
            try renderToken(r, rparen, .space, options); // )
        } else {
            try renderToken(r, rparen, .none, options); // )
            return renderToken(r, rparen + 1, Space.newline, options); // ;
        }
    }
    ...
    if (var_decl.ast.section_node.unwrap()) |section_node| {
        ...
        try renderToken(r, section_kw, Space.none, options); // linksection
        try renderToken(r, lparen, Space.none, options); // (
        try renderExpression(r, section_node, Space.none, options);
    ...
    try ais.pushIndent(.after_equals);
    try renderToken(r, eq_token, eq_space, options); // =
    try renderExpression(r, init_node, space, options); // ;
    ais.popIndent();
}

/// Render an if expression by delegating to the renderWhile function with appropriate parameters.
fn renderIf(
    r: *Render,
    if_node: Ast.full.If,
    space: Space,
...
fn renderWhile(r: *Render, while_node: Ast.full.While, space: Space, options: Options) Error!void {
    const tree = r.tree;

    if (while_node.label_token) |label| {
        try renderIdentifier(r, label, .none, .eagerly_unquote, options); // label
        try renderToken(r, label + 1, .space, options); // :
    }

    if (while_node.inline_token) |inline_token| {
        try renderToken(r, inline_token, .space, options); // inline
    }

    try renderToken(r, while_node.ast.while_token, .space, options); // if/for/while
    try renderToken(r, while_node.ast.while_token + 1, .none, options); // lparen
    try renderExpression(r, while_node.ast.cond_expr, .none, options); // condition

    var last_prefix_token = tree.lastToken(while_node.ast.cond_expr) + 1; // rparen

    if (while_node.payload_token) |payload_token| {
        try renderToken(r, last_prefix_token, .space, options);
        try renderToken(r, payload_token - 1, .none, options); // |
        const ident = blk: {
            if (tree.tokenTag(payload_token) == .asterisk) {
                try renderToken(r, payload_token, .none, options); // *
                break :blk payload_token + 1;
            } else {
                break :blk payload_token;
            }
        };
        try renderIdentifier(r, ident, .none, .preserve_when_shadowing, options); // identifier
        const pipe = blk: {
            if (tree.tokenTag(ident + 1) == .comma) {
                try renderToken(r, ident + 1, .space, options); // ,
                try renderIdentifier(r, ident + 2, .none, .preserve_when_shadowing, options); // index
                break :blk ident + 3;
            } else {
                break :blk ident + 1;
            }
    ...
    if (while_node.ast.cont_expr.unwrap()) |cont_expr| {
        ...
        try renderToken(r, lparen - 1, .space, options); // :
        try renderToken(r, lparen, .none, options); // lparen
        try renderExpression(r, cont_expr, .none, options);
        last_prefix_token = tree.lastToken(cont_expr) + 1; // rparen
    }

    try renderThenElse(r, last_prefix_token, while_node.ast.then_expr, while_node.else_token, while_node.error_token, while_node.ast.else_expr, space, options);
}

/// Render the then and else branches of an if/while/for expression with proper indentation.
fn renderThenElse(
    r: *Render,
    ...
    if (then_expr_is_block and ais.isLineOverIndented()) {
        ...
        try renderToken(r, last_prefix_token, .newline, options);
    ...
    if (opt_else_expr.unwrap()) |else_expr| {
        if (indent_then_expr) {
            try renderExpression(r, then_expr, .newline, options);
        } else {
            try renderExpression(r, then_expr, .space, options);
        }
        ...
        if (maybe_error_token) |error_token| {
            try renderToken(r, last_else_token, .space, options); // else
            try renderToken(r, error_token - 1, .none, options); // |
            try renderIdentifier(r, error_token, .none, .preserve_when_shadowing, options); // identifier
        ...
        if (indent_else_expr) {
            try ais.pushIndent(.normal);
            try renderToken(r, last_else_token, .newline, options);
            try renderExpression(r, else_expr, space, options);
            ais.popIndent();
        } else {
            try renderToken(r, last_else_token, .space, options);
            try renderExpression(r, else_expr, space, options);
        }
    } else {
        try renderExpression(r, then_expr, space, options);
...
/// Render a for loop expression with optional label, inline keyword, and payload capture.
fn renderFor(
    r: *Render,
    for_node: Ast.full.For,
    space: Space,
    options: Options,
) Error!void {
    const tree = r.tree;
    ...
    try renderToken(r, for_node.ast.for_token, .space, options); // if/for/while

    const lparen = for_node.ast.for_token + 1;
    try renderParamList(r, lparen, for_node.ast.inputs, .space, options);
    ...
    if (tree.tokenTag(@intCast(pipe - 1)) == .comma) {
        try ais.pushIndent(.normal);
        try renderToken(r, cur - 1, .newline, options); // |
        while (true) {
            if (tree.tokenTag(cur) == .asterisk) {
                try renderToken(r, cur, .none, options); // *
                cur += 1;
            }
            try renderIdentifier(r, cur, .none, .preserve_when_shadowing, options); // identifier
            cur += 1;
            if (tree.tokenTag(cur) == .comma) {
                try renderToken(r, cur, .newline, options); // ,
                cur += 1;
            }
        ...
        try renderToken(r, cur - 1, .none, options); // |
        while (true) {
            if (tree.tokenTag(cur) == .asterisk) {
                try renderToken(r, cur, .none, options); // *
                cur += 1;
            ...
            cur += 1;
            if (tree.tokenTag(cur) == .comma) {
                try renderToken(r, cur, .space, options); // ,
                cur += 1;
            }
            if (tree.tokenTag(cur) == .pipe) {
                break;
    ...
    try renderThenElse(r, cur, for_node.ast.then_expr, for_node.else_token, null, for_node.ast.else_expr, space, options);
}

/// Render a container field (struct/union/enum field, options) with proper formatting based on container type.
fn renderContainerField(
    ...
    if (field.comptime_token) |t| {
        try renderToken(r, t, .space, options); // comptime
    }
    if (field.ast.type_expr == .none and field.ast.value_expr == .none) {
        if (field.ast.align_expr.unwrap()) |align_expr| {
            try renderIdentifier(r, field.ast.main_token, .space, quote, options); // name
            ...
            try renderToken(r, align_kw, .none, options); // align
            try renderToken(r, lparen_token, .none, options); // (
            try renderExpression(r, align_expr, .none, options); // alignment
            return renderToken(r, rparen_token, .space, options); // )
        }
        return renderIdentifierComma(r, field.ast.main_token, space, quote, options); // name
    }
    if (field.ast.type_expr != .none and field.ast.value_expr == .none) {
        const type_expr = field.ast.type_expr.unwrap().?;
        if (!field.ast.tuple_like) {
            try renderIdentifier(r, field.ast.main_token, .none, quote, options); // name
            try renderToken(r, field.ast.main_token + 1, .space, options); // :
        }

        if (field.ast.align_expr.unwrap()) |align_expr| {
            try renderExpression(r, type_expr, .space, options); // type
            const align_token = tree.firstToken(align_expr) - 2;
            try renderToken(r, align_token, .none, options); // align
            try renderToken(r, align_token + 1, .none, options); // (
            try renderExpression(r, align_expr, .none, options); // alignment
            const rparen = tree.lastToken(align_expr) + 1;
            return renderTokenComma(r, rparen, space, options); // )
        } else {
            return renderExpressionComma(r, type_expr, space, options); // type
        }
    }
    if (field.ast.type_expr == .none and field.ast.value_expr != .none) {
        ...
        if (field.ast.align_expr.unwrap()) |align_expr| {
            ...
            try renderToken(r, lparen_token, .none, options); // (
            try renderExpression(r, align_expr, .none, options); // alignment
            try renderToken(r, rparen_token, .space, options); // )
        }
        try renderToken(r, field.ast.main_token + 1, .space, options); // =
        return renderExpressionComma(r, value_expr, space, options); // value
    }
    if (!field.ast.tuple_like) {
        try renderIdentifier(r, field.ast.main_token, .none, quote, options); // name
        try renderToken(r, field.ast.main_token + 1, .space, options); // :
    ...
    if (eq_space == .space) {
        ais.popIndent();
        try renderExpressionComma(r, value_expr, space, options); // value
        return;
    }
    ...
    if (tree.tokenTag(maybe_comma) == .comma) {
        try renderExpression(r, value_expr, .none, options); // value
        ais.popIndent();
        try renderToken(r, maybe_comma, .newline, options);
    } else {
        try renderExpression(r, value_expr, space, options); // value
        ais.popIndent();
    }
}

/// Render a builtin function call with special handling for @import path rebasing and cast canonicalization.
fn renderBuiltinCall(
    ...
    try renderToken(r, builtin_token, .none, options); // @name

    if (params.len == 0) {
        try renderToken(r, builtin_token + 1, .none, options); // (
        return renderToken(r, builtin_token + 2, space, options); // )
    }
    ...
    if (r.fixups.rebase_imported_paths) |prefix| {
        ...
        if (mem.eql(u8, slice, "@import")) f: {
            ...
            return renderToken(r, str_lit_token + 1, space, options); // )
        }
    }
    ...
    if (tree.tokenTag(after_last_param_token) != .comma) {
        ...
        for (params, 0..) |param_node, i| {
            ...
            {
                try ais.pushIndent(.normal);
                try renderExpression(r, param_node, .none, options);
                ais.popIndent();
            } else {
                try renderExpression(r, param_node, .none, options);
            }
            ...
            if (i + 1 < params.len) {
                ...
                try renderToken(r, comma_token, .space, options); // ,
            }
        }
        return renderToken(r, after_last_param_token, space, options); // )
    } else {
        // Render one param per line.
        try ais.pushIndent(.normal);
        try renderToken(r, builtin_token + 1, Space.newline, options); // (

        for (params) |param_node| {
            try ais.pushSpace(.comma);
            try renderExpression(r, param_node, .comma, options);
            ais.popSpace();
        }
        ais.popIndent();

        return renderToken(r, after_last_param_token + 1, space, options); // )
    }
}

/// Render a function prototype with parameters, return type, and optional attributes.
fn renderFnProto(r: *Render, fn_proto: Ast.full.FnProto, space: Space, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;

    const after_fn_token = fn_proto.ast.fn_token + 1;
    const lparen = if (tree.tokenTag(after_fn_token) == .identifier) blk: {
        try renderToken(r, fn_proto.ast.fn_token, .space, options); // fn
        try renderIdentifier(r, after_fn_token, .none, .preserve_when_shadowing, options); // name
    ...
    if (!trailing_comma and !hasComment(tree, lparen, rparen)) {
        // Render all on one line, no trailing comma.
        try renderToken(r, lparen, .none, options); // (
        ...
        while (true) {
            ...
            switch (tree.tokenTag(last_param_token)) {
                .doc_comment => {
                    try renderToken(r, last_param_token, .newline, options);
                    continue;
                },
                .ellipsis3 => {
                    try renderToken(r, last_param_token, .none, options); // ...
                    break;
                },
                .keyword_noalias, .keyword_comptime => {
                    try renderToken(r, last_param_token, .space, options);
                    last_param_token += 1;
                },
                .identifier => {},
                .keyword_anytype => {
                    try renderToken(r, last_param_token, .none, options); // anytype
                    continue;
                },
                .r_paren => break,
                .comma => {
            ...
            {
                try renderIdentifier(r, last_param_token, .none, .preserve_when_shadowing, options); // name
            ...
            param_i += 1;
            try renderExpression(r, param, .none, options);
            last_param_token = tree.lastToken(param);
        }
    } else {
        // One param per line.
        try ais.pushIndent(.normal);
        try renderToken(r, lparen, .newline, options); // (
        ...
        while (true) {
            ...
            switch (tree.tokenTag(last_param_token)) {
                ...
                .ellipsis3 => {
                    try renderToken(r, last_param_token, .comma, options); // ...
                    break;
                },
                .keyword_noalias, .keyword_comptime => {
                    try renderToken(r, last_param_token, .space, options);
                ...
                .identifier => {},
                .keyword_anytype => {
                    try renderToken(r, last_param_token, .comma, options); // anytype
            ...
            param_i += 1;
            try ais.pushSpace(.comma);
            try renderExpression(r, param, .comma, options);
            ais.popSpace();
            last_param_token = tree.lastToken(param);
    ...
    if (fn_proto.ast.align_expr.unwrap()) |align_expr| {
        ...
        try renderToken(r, align_lparen - 1, .none, options); // align
        try renderToken(r, align_lparen, .none, options); // (
        try renderExpression(r, align_expr, .none, options);
        try renderToken(r, align_rparen, .space, options); // )
    }
    ...
    if (fn_proto.ast.addrspace_expr.unwrap()) |addrspace_expr| {
        ...
        try renderToken(r, align_lparen - 1, .none, options); // addrspace
        try renderToken(r, align_lparen, .none, options); // (
        try renderExpression(r, addrspace_expr, .none, options);
        try renderToken(r, align_rparen, .space, options); // )
    }
    ...
    if (fn_proto.ast.section_expr.unwrap()) |section_expr| {
        ...
        try renderToken(r, section_lparen - 1, .none, options); // section
        try renderToken(r, section_lparen, .none, options); // (
        try renderExpression(r, section_expr, .none, options);
        try renderToken(r, section_rparen, .space, options); // )
    }
    ...
    if (fn_proto.ast.callconv_expr.unwrap()) |callconv_expr| {
        ...
        if (!(is_declaration and is_callconv_inline)) {
            ...
            try renderToken(r, callconv_lparen - 1, .none, options); // callconv
            try renderToken(r, callconv_lparen, .none, options); // (
            try renderExpression(r, callconv_expr, .none, options);
            try renderToken(r, callconv_rparen, .space, options); // )
        }
    }

    if (tree.tokenTag(maybe_bang) == .bang) {
        try renderToken(r, maybe_bang, .none, options); // !
    }
    return renderExpression(r, return_type, space, options);
}

/// Render a switch case with optional inline keyword, values, payload capture, and target expression.
fn renderSwitchCase(
    r: *Render,
    ...
    if (switch_case.inline_token) |some| {
        try renderToken(r, some, .space, options);
    }

    // Render everything before the arrow
    if (switch_case.ast.values.len == 0) {
        try renderToken(r, switch_case.ast.arrow_token - 1, .space, options); // else keyword
    } else if (trailing_comma or has_comment_before_arrow) {
        // Render each value on a new line
        try ais.pushSpace(.comma);
        try renderExpressions(r, switch_case.ast.values, .comma, options);
        ais.popSpace();
    } else {
        // Render on one line
        for (switch_case.ast.values) |value_expr| {
            try renderExpression(r, value_expr, .comma_space, options);
        }
    }
    ...
    try renderToken(r, switch_case.ast.arrow_token, after_arrow_space, options); // =>

    if (switch_case.payload_token) |payload_token| {
        try renderToken(r, payload_token - 1, .none, options); // pipe
        const ident = payload_token + @intFromBool(tree.tokenTag(payload_token) == .asterisk);
        if (tree.tokenTag(payload_token) == .asterisk) {
            try renderToken(r, payload_token, .none, options); // asterisk
        }
        try renderIdentifier(r, ident, .none, .preserve_when_shadowing, options); // identifier
        if (tree.tokenTag(ident + 1) == .comma) {
            try renderToken(r, ident + 1, .space, options); // ,
            try renderIdentifier(r, ident + 2, .none, .preserve_when_shadowing, options); // identifier
            try renderToken(r, ident + 3, pre_target_space, options); // pipe
        } else {
            try renderToken(r, ident + 1, pre_target_space, options); // pipe
        }
    }

    try renderExpression(r, switch_case.ast.target_expr, space, options);
}

/// Render a block expression with proper indentation and statement formatting.
fn renderBlock(r: *Render, block_node: Ast.Node.Index, statements: []const Ast.Node.Index, space: Space, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    const lbrace = tree.nodeMainToken(block_node);

    if (tree.isTokenPrecededByTags(lbrace, &.{ .identifier, .colon })) {
        try renderIdentifier(r, lbrace - 2, .none, .eagerly_unquote, options); // identifier
        try renderToken(r, lbrace - 1, .space, options); // :
    }

    if (options.brace_style == .allman) {
        try ais.insertNewline();
        try renderToken(r, lbrace, .none, options); // `{` on its own line

        if (statements.len == 0) {
            try ais.maybeInsertNewline();
            try renderToken(r, tree.lastToken(block_node), .none, options); // rbrace
            return;
        }

        try ais.pushIndent(.normal); // indent inner
        try ais.insertNewline();
        return finishRenderBlock(r, block_node, statements, space, options);
    }
    ...
    if (statements.len == 0) {
        try renderToken(r, lbrace, .none, options);
        ais.popIndent();
        try renderToken(r, tree.lastToken(block_node), space, options); // rbrace
        return;
    }
    try renderToken(r, lbrace, .newline, options);
    return finishRenderBlock(r, block_node, statements, space, options);
...
/// Complete the rendering of a block by processing all statements and closing the block.
fn finishRenderBlock(
    ...
    for (statements, 0..) |stmt, i| {
        if (i != 0) try renderExtraNewline(r, stmt, options);
        ...
        switch (tree.nodeTag(stmt)) {
            ...
            .simple_var_decl,
            .aligned_var_decl,
            => try renderVarDecl(r, tree.fullVarDecl(stmt).?, false, .semicolon, options),

            else => try renderExpression(r, stmt, .semicolon, options),
        }
        ais.popSpace();
    }

    // Pop indent before printing the closing brace so `}` it aligns with the opening
    ais.popIndent();

    if (options.brace_style == .allman) {
        try ais.maybeInsertNewline(); // <- avoids double-blank-line
        try renderToken(r, tree.lastToken(block_node), .none, options); // rbrace
    } else {
        try renderToken(r, tree.lastToken(block_node), space, options); // rbrace
    }
...
/// Render a struct initialization expression with field assignments.
fn renderStructInit(
    r: *Render,
    ...
    if (struct_init.ast.type_expr.unwrap()) |type_expr| {
        try renderExpression(r, type_expr, .none, options); // T
    } else {
        try renderToken(r, struct_init.ast.lbrace - 1, .none, options); // .
    }

    if (struct_init.ast.fields.len == 0) {
        try ais.pushIndent(.normal);
        try renderToken(r, struct_init.ast.lbrace, .none, options); // lbrace
        ais.popIndent();
        return renderToken(r, struct_init.ast.lbrace + 1, space, options); // rbrace
    }
    ...
    if (trailing_comma or hasComment(tree, struct_init.ast.lbrace, rbrace)) {
        ...
        try ais.pushIndent(.normal);
        try renderToken(r, struct_init.ast.lbrace, .newline, options);

        try renderToken(r, struct_init.ast.lbrace + 1, .none, options); // .
        try renderIdentifier(r, struct_init.ast.lbrace + 2, .space, .eagerly_unquote, options); // name
        ...
        try renderToken(r, struct_init.ast.lbrace + 3, space_after_equal, options); // =

        try ais.pushSpace(.comma);
        try renderExpressionFixup(r, field_node, .comma, options);
        ais.popSpace();

        for (struct_init.ast.fields[1..]) |field_init| {
            const init_token = tree.firstToken(field_init);
            try renderExtraNewlineToken(r, init_token - 3, options);
            try renderToken(r, init_token - 3, .none, options); // .
            try renderIdentifier(r, init_token - 2, .space, .eagerly_unquote, options); // name
            space_after_equal = if (tree.nodeTag(field_init) == .multiline_string_literal) .none else .space;
            try renderToken(r, init_token - 1, space_after_equal, options); // =

            try ais.pushSpace(.comma);
            try renderExpressionFixup(r, field_init, .comma, options);
            ais.popSpace();
        }
        ...
        // Render all on one line, no trailing comma.
        try renderToken(r, struct_init.ast.lbrace, .space, options);

        for (struct_init.ast.fields) |field_init| {
            const init_token = tree.firstToken(field_init);
            try renderToken(r, init_token - 3, .none, options); // .
            try renderIdentifier(r, init_token - 2, .space, .eagerly_unquote, options); // name
            try renderToken(r, init_token - 1, .space, options); // =
            try renderExpressionFixup(r, field_init, .comma_space, options);
        }
    }
...
/// Render an array initialization expression with intelligent column-based formatting.
fn renderArrayInit(
    r: *Render,
    ...
    if (array_init.ast.type_expr.unwrap()) |type_expr| {
        ...
        try renderToken(r, array_init.ast.lbrace - 1, .none, options); // .
    }

    if (array_init.ast.elements.len == 0) {
        try ais.pushIndent(.normal);
        try renderToken(r, array_init.ast.lbrace, .none, options); // lbrace
        ais.popIndent();
        return renderToken(r, array_init.ast.lbrace + 1, space, options); // rbrace
    }
    ...
    if (array_init.ast.elements.len == 1) {
        ...
        {
            try renderToken(r, array_init.ast.lbrace, .none, options);
            try renderExpression(r, only_elem, .none, options);
            return renderToken(r, rbrace, space, options);
        }
    }
    ...
    if (!trailing_comma and !contains_comment and !contains_multiline_string) {
        ...
        if (array_init.ast.elements.len == 1) {
            ...
            try renderToken(r, array_init.ast.lbrace, .none, options);
            try renderExpression(r, array_init.ast.elements[0], .none, options);
        } else {
            try renderToken(r, array_init.ast.lbrace, .space, options);
            for (array_init.ast.elements) |elem| {
                try renderExpression(r, elem, .comma_space, options);
            }
        }
        return renderToken(r, last_elem_token + 1, space, options); // rbrace
    }

    try ais.pushIndent(.normal);
    try renderToken(r, array_init.ast.lbrace, .newline, options);
    ...
    while (true) {
        ...
        var auto_indenting_stream: AutoIndentingStream = .init(gpa, &sub_expr_buffer.writer, options.indent_delta);
        defer auto_indenting_stream.deinit();
        ...
        for (section_exprs, 0..) |expr, i| {
            ...
            if (i + 1 < section_exprs.len) {
                try renderExpression(&sub_render, expr, .none, options);
                ...
                try ais.pushSpace(.comma);
                try renderExpression(&sub_render, expr, .comma, options);
                ais.popSpace();
            ...
            if (i + 1 < section_exprs.len) {
                ...
                if (column_counter != row_size - 1) {
                    if (!expr_newlines[i] and !expr_newlines[i + 1]) {
                        ...
                        try renderToken(r, comma, .space, options); // ,
                ...
                column_counter = 0;
                try renderToken(r, comma, .newline, options); // ,
                try renderExtraNewline(r, next_expr, options);
            }
        }
    ...
    ais.popIndent();
    return renderToken(r, rbrace, space, options); // rbrace
}

/// Render a container declaration (struct/union/enum, options) with all its members.
fn renderContainerDecl(
    ...
    if (container_decl.layout_token) |layout_token| {
        try renderToken(r, layout_token, .space, options);
    }
    ...
    if (container_decl.ast.enum_token) |enum_token| {
        try renderToken(r, container_decl.ast.main_token, .none, options); // union
        try renderToken(r, enum_token - 1, .none, options); // lparen
        try renderToken(r, enum_token, .none, options); // enum
        if (container_decl.ast.arg.unwrap()) |arg| {
            try renderToken(r, enum_token + 1, .none, options); // lparen
            try renderExpression(r, arg, .none, options);
            const rparen = tree.lastToken(arg) + 1;
            try renderToken(r, rparen, .none, options); // rparen
            try renderToken(r, rparen + 1, .space, options); // rparen
            lbrace = rparen + 2;
        } else {
            try renderToken(r, enum_token + 1, .space, options); // rparen
            lbrace = enum_token + 2;
        }
    } else if (container_decl.ast.arg.unwrap()) |arg| {
        try renderToken(r, container_decl.ast.main_token, .none, options); // union
        try renderToken(r, container_decl.ast.main_token + 1, .none, options); // lparen
        try renderExpression(r, arg, .none, options);
        const rparen = tree.lastToken(arg) + 1;
        try renderToken(r, rparen, .space, options); // rparen
        lbrace = rparen + 1;
    } else {
        try renderToken(r, container_decl.ast.main_token, .space, options); // union
    ...
    if (container_decl.ast.members.len == 0) {
        ...
        if (tree.tokenTag(lbrace + 1) == .container_doc_comment) {
            try renderToken(r, lbrace, .newline, options); // lbrace
            try renderContainerDocComments(r, lbrace + 1, options);
        } else {
            try renderToken(r, lbrace, .none, options); // lbrace
        }
        ais.popIndent();
        return renderToken(r, rbrace, space, options); // rbrace
    }
    ...
    if (!src_has_trailing_comma) one_line: {
        ...
        try renderToken(r, lbrace, .space, options); // lbrace
        for (container_decl.ast.members) |member| {
            try renderMember(r, container, member, .space, options);
        }
        return renderToken(r, rbrace, space, options); // rbrace
    }
    ...
    for (container_decl.ast.members, 0..) |member, i| {
        if (i != 0) try renderExtraNewline(r, member, options);
        switch (tree.nodeTag(member)) {
            ...
            => {
                try ais.pushSpace(.comma);
                try renderMember(r, container, member, .comma, options);
                ais.popSpace();
            },

            else => try renderMember(r, container, member, .newline, options),
        }
    }
    ais.popIndent();
...
/// Render legacy assembly syntax with template, outputs, inputs, and clobbers.
fn renderAsmLegacy(
    r: *Render,
    ...
    try renderToken(r, asm_node.ast.asm_token, .space, options); // asm

    if (asm_node.volatile_token) |volatile_token| {
        try renderToken(r, volatile_token, .space, options); // volatile
        try renderToken(r, volatile_token + 1, .none, options); // lparen
    } else {
        try renderToken(r, asm_node.ast.asm_token + 1, .none, options); // lparen
    }
    ...
    if (asm_node.ast.items.len == 0) {
        ...
        if (asm_node.first_clobber) |first_clobber| {
            // asm ("foo" ::: "a", "b")
            // asm ("foo" ::: "a", "b",)
            try renderExpression(r, asm_node.ast.template, .space, options);
            // Render the three colons.
            try renderToken(r, first_clobber - 3, .none, options);
            try renderToken(r, first_clobber - 2, .none, options);
            try renderToken(r, first_clobber - 1, .space, options);
            ...
            while (true) : (tok_i += 1) {
                try ais.writeByte('.');
                _ = try writeStringLiteralAsIdentifier(r, tok_i, options);
                ...
                switch (tree.tokenTag(tok_i)) {
                    .r_paren => {
                        try ais.writeAll(" }");
                        ais.popIndent();
                        return renderToken(r, tok_i, space, options);
                    },
                    .comma => {
                        if (tree.tokenTag(tok_i + 1) == .r_paren) {
                            try ais.writeAll(" }");
                            ais.popIndent();
                            return renderToken(r, tok_i + 1, space, options);
                        } else {
                            try renderToken(r, tok_i, .space, options);
                        }
                    },
                    else => unreachable,
                }
    ...
    try ais.forcePushIndent(.normal);
    try renderExpression(r, asm_node.ast.template, .newline, options);
    ais.setIndentDelta(options.asm_indent_delta);
    const colon1 = tree.lastToken(asm_node.ast.template) + 1;

    const colon2 = if (asm_node.outputs.len == 0) colon2: {
        try renderToken(r, colon1, .newline, options); // :
        break :colon2 colon1 + 1;
    } else colon2: {
        try renderToken(r, colon1, .space, options); // :

        try ais.forcePushIndent(.normal);
        for (asm_node.outputs, 0..) |asm_output, i| {
            if (i + 1 < asm_node.outputs.len) {
                const next_asm_output = asm_node.outputs[i + 1];
                try renderAsmOutput(r, asm_output, .none, options);

                const comma = tree.firstToken(next_asm_output) - 1;
                try renderToken(r, comma, .newline, options); // ,
                try renderExtraNewlineToken(r, tree.firstToken(next_asm_output), options);
            } else if (asm_node.inputs.len == 0 and asm_node.first_clobber == null) {
                try ais.pushSpace(.comma);
                try renderAsmOutput(r, asm_output, .comma, options);
                ais.popSpace();
                ais.popIndent();
                ais.setIndentDelta(options.indent_delta);
                ais.popIndent();
                return renderToken(r, asm_node.ast.rparen, space, options); // rparen
            } else {
                try ais.pushSpace(.comma);
                try renderAsmOutput(r, asm_output, .comma, options);
                ais.popSpace();
    ...
    const colon3 = if (asm_node.inputs.len == 0) colon3: {
        try renderToken(r, colon2, .newline, options); // :
        break :colon3 colon2 + 1;
    } else colon3: {
        try renderToken(r, colon2, .space, options); // :
        try ais.forcePushIndent(.normal);
        for (asm_node.inputs, 0..) |asm_input, i| {
            if (i + 1 < asm_node.inputs.len) {
                const next_asm_input = asm_node.inputs[i + 1];
                try renderAsmInput(r, asm_input, .none, options);

                const first_token = tree.firstToken(next_asm_input);
                try renderToken(r, first_token - 1, .newline, options); // ,
                try renderExtraNewlineToken(r, first_token, options);
            } else if (asm_node.first_clobber == null) {
                try ais.pushSpace(.comma);
                try renderAsmInput(r, asm_input, .comma, options);
                ais.popSpace();
                ais.popIndent();
                ais.setIndentDelta(options.indent_delta);
                ais.popIndent();
    ...
    try renderToken(r, colon3, .space, options); // :
    try ais.writeAll(".{ ");
    ...
    while (true) {
        switch (tree.tokenTag(tok_i + 1)) {
            .r_paren => {
                ais.setIndentDelta(options.indent_delta);
                try ais.writeByte('.');
                const lexeme_len = try writeStringLiteralAsIdentifier(r, tok_i, options);
                try ais.writeAll(" = true }");
                try renderSpace(r, tok_i, lexeme_len, .newline, options);
                ais.popIndent();
                return renderToken(r, tok_i + 1, space, options);
            },
            .comma => {
                switch (tree.tokenTag(tok_i + 2)) {
                    .r_paren => {
                        ...
                        try renderSpace(r, tok_i, lexeme_len, .newline, options);
                        ais.popIndent();
                        return renderToken(r, tok_i + 2, space, options);
                    },
                    else => {
                        try ais.writeByte('.');
                        _ = try writeStringLiteralAsIdentifier(r, tok_i, options);
                        try ais.writeAll(" = true");
                        try renderToken(r, tok_i + 1, .space, options);
                        tok_i += 2;
                    },
                }
            },
...
/// Render modern assembly syntax with template, outputs, inputs, and clobbers.
fn renderAsm(r: *Render, asm_node: Ast.full.Asm, space: Space, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    ...
    if (asm_node.ast.items.len == 0) {
        ...
        if (asm_node.ast.clobbers.unwrap()) |clobbers| {
            ...
            try renderToken(r, first_clobber - 2, .none, options);
            try renderToken(r, first_clobber - 1, .space, options);
            try renderExpression(r, clobbers, .none, options);
            ais.popIndent();
            return renderToken(r, asm_node.ast.rparen, space, options); // rparen
        }

        // asm ("foo")
        try renderExpression(r, asm_node.ast.template, .none, options);
        ais.popIndent();
        return renderToken(r, asm_node.ast.rparen, space, options); // rparen
    }
...
/// Render a function call expression with the function and its parameters.
fn renderCall(
    r: *Render,
    call: Ast.full.Call,
    space: Space,
    ...
    try renderExpression(r, call.ast.fn_expr, .none, options);
    try renderParamList(r, call.ast.lparen, call.ast.params, space, options);
}

/// Render a parameter list for function calls with proper formatting and trailing comma handling.
fn renderParamList(
    r: *Render,
    ...
    if (params.len == 0) {
        ...
        try renderToken(r, lparen, .none, options);
        ais.popIndent();
        return renderToken(r, lparen + 1, space, options); // )
    }
    ...
    if (tree.tokenTag(after_last_param_tok) == .comma) {
        ...
        for (params, 0..) |param_node, i| {
            if (i + 1 < params.len) {
                ...
                try renderExtraNewline(r, params[i + 1], options);
            } else {
                try ais.pushSpace(.comma);
                try renderExpression(r, param_node, .comma, options);
                ais.popSpace();
        ...
        ais.popIndent();
        return renderToken(r, after_last_param_tok + 1, space, options); // )
    }
    ...
    for (params, 0..) |param_node, i| {
        ...
        if (i + 1 < params.len) {
            ...
            try renderToken(r, comma, comma_space, options);
        }
    }
    ais.popIndent();
    return renderToken(r, after_last_param_tok, space, options); // )
}

/// Render an expression, and the comma that follows it, if it is present in the source.
/// If a comma is present, and `space` is `Space.comma`, render only a single comma.
fn renderExpressionComma(
    r: *Render,
    node: Ast.Node.Index,
    space: Space,
    options: Options,
    ...
    if (tree.tokenTag(maybe_comma) == .comma and space != .comma) {
        try renderExpression(r, node, .none, options);
        return renderToken(r, maybe_comma, space, options);
    } else {
        return renderExpression(r, node, space, options);
    }
}
...
fn renderTokenComma(
    ...
    if (tree.tokenTag(maybe_comma) == .comma and space != .comma) {
        try renderToken(r, token, .none, options);
        return renderToken(r, maybe_comma, space, options);
    } else {
        return renderToken(r, token, space, options);
    }
}

/// Render an identifier, and the comma that follows it, if it is present in the source.
/// If a comma is present, and `space` is `Space.comma`, render only a single comma.
fn renderIdentifierComma(
    r: *Render,
    token: Ast.TokenIndex,
    space: Space,
    quote: QuoteBehavior,
    ...
    const maybe_comma = token + 1;
    if (tree.tokenTag(maybe_comma) == .comma and space != .comma) {
        try renderIdentifier(r, token, .none, quote, options);
        return renderToken(r, maybe_comma, space, options);
    } else {
        return renderIdentifier(r, token, space, quote, options);
    }
}
...
/// Render a single token with appropriate spacing based on the space parameter.
fn renderToken(r: *Render, token_index: Ast.TokenIndex, space: Space, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    const lexeme = tokenSliceForRender(tree, token_index, options);
    try ais.writeAll(lexeme);
    try renderSpace(r, token_index, lexeme.len, space, options);
}

/// Render a token with a temporary space mode override for special formatting cases.
...
/// Render whitespace and comments after a token based on the space parameter.
fn renderSpace(
    r: *Render,
    token_index: Ast.TokenIndex,
    lexeme_len: usize,
    ...
    switch (space) {
        ...
        .comma => if (next_token_tag == .comma) {
            try renderToken(r, token_index + 1, .newline, options);
        } else if (!comment) {
            try ais.insertNewline();
        },

        .comma_space => if (next_token_tag == .comma) {
            try renderToken(r, token_index + 1, .space, options);
        } else if (!comment) {
            try ais.writeByte(' ');
        },
...
/// Render only whitespace without any token content.
...
/// Render an identifier with optional unquoting and renaming based on fixups.
fn renderIdentifier(r: *Render, token_index: Ast.TokenIndex, space: Space, quote: QuoteBehavior, options: Options) Error!void {
    const tree = r.tree;
    ...
    if (lexeme[0] != '@') {
        return renderToken(r, token_index, space, options);
    }
    ...
    if (contents.len == 0) {
        return renderQuotedIdentifier(r, token_index, space, false, options);
    }

    // Special case for _.
    if (std.zig.isUnderscore(contents)) switch (quote) {
        .eagerly_unquote => return renderQuotedIdentifier(r, token_index, space, true, options),
        .eagerly_unquote_except_underscore,
        .preserve_when_shadowing,
        => return renderQuotedIdentifier(r, token_index, space, false, options),
    };
    ...
    while (contents_i < contents.len) {
        switch (contents[contents_i]) {
            '0'...'9' => if (contents_i == 0) return renderQuotedIdentifier(r, token_index, space, false, options),
            'A'...'Z', 'a'...'z', '_' => {},
            '\\' => {
                ...
                switch (res) {
                    .success => |char| switch (char) {
                        '0'...'9' => if (contents_i == 0) return renderQuotedIdentifier(r, token_index, space, false, options),
                        'A'...'Z', 'a'...'z', '_' => {},
                        else => return renderQuotedIdentifier(r, token_index, space, false, options),
                    },
                    .failure => return renderQuotedIdentifier(r, token_index, space, false, options),
                }
    ...
    if (contents_i == contents.len) {
        ...
        if (primitives.isPrimitive(buf[0..buf_i])) switch (quote) {
            .eagerly_unquote,
            .eagerly_unquote_except_underscore,
            => return renderQuotedIdentifier(r, token_index, space, true, options),
            .preserve_when_shadowing => return renderQuotedIdentifier(r, token_index, space, false, options),
        };
    }

    try renderQuotedIdentifier(r, token_index, space, true, options);
}
...
fn renderQuotedIdentifier(r: *Render, token_index: Ast.TokenIndex, space: Space, comptime unquote: bool, options: Options) !void {
    const tree = r.tree;
    const ais = r.ais;
    assert(tree.tokenTag(token_index) == .identifier);
    const lexeme = tokenSliceForRender(tree, token_index, options);
...
/// Render the contents of a quoted identifier, handling escape sequences and normalization.
...
fn renderComments(r: *Render, start: usize, end: usize, options: Options) Error!bool {
    const tree = r.tree;
    const ais = r.ais;
    ...
    while (mem.indexOf(u8, tree.source[index..end], "//")) |offset| {
        ...
        if (ais.disabled_offset != null and mem.eql(u8, comment_content, "zig fmt: on")) {
            ...
            try writeFixingWhitespace(ais.underlying_writer, disabled_source, options);
...
/// Check for and render an extra newline before a node if one exists in the source.
fn renderExtraNewline(r: *Render, node: Ast.Node.Index, options: Options) Error!void {
    return renderExtraNewlineToken(r, r.tree.firstToken(node), options);
}

/// Check if there is an empty line immediately before the given token. If so, render it.
fn renderExtraNewlineToken(r: *Render, token_index: Ast.TokenIndex, options: Options) Error!void {
    const tree = r.tree;
    const ais = r.ais;
    ...
    const prev_token_end = if (token_index == 0)
        ...
        tree.tokenStart(token_index - 1) + tokenSliceForRender(tree, token_index - 1, options).len;
...
fn renderDocComments(r: *Render, end_token: Ast.TokenIndex, options: Options) Error!void {
    const tree = r.tree;
    ...
    if (first_tok != 0) {
        ...
        if (prev_token_tag != .l_brace) {
            try renderExtraNewlineToken(r, first_tok, options);
        }
    }

    while (tree.tokenTag(tok) == .doc_comment) : (tok += 1) {
        try renderToken(r, tok, .newline, options);
    }
}

/// start_token is first container doc comment token.
fn renderContainerDocComments(r: *Render, start_token: Ast.TokenIndex, options: Options) Error!void {
    const tree = r.tree;
    var tok = start_token;
    ...
    if (tree.tokenTag(tok) != .doc_comment) {
        try renderExtraNewlineToken(r, tok, options);
    }
}

/// Generate discard statements for all function parameters to avoid unused variable warnings.
fn discardAllParams(r: *Render, fn_proto_node: Ast.Node.Index, options: Options) Error!void {
    const tree = &r.tree;
    const ais = r.ais;
    var buf: [1]Ast.Node.Index = undefined;
...
/// Get the token slice for rendering, with special handling for doc comments.
fn tokenSliceForRender(tree: Ast, token_index: Ast.TokenIndex, options: Options) []const u8 {
    _ = options;
...
/// Write a string literal as an identifier, adding @ prefix if needed for validity.
fn writeStringLiteralAsIdentifier(r: *Render, token_index: Ast.TokenIndex, options: Options) !usize {
    const tree = r.tree;
    const ais = r.ais;
...
/// Check if there is a comment on the same line as the given token.
...
/// Write source code with tabs converted to spaces and carriage returns removed.
fn writeFixingWhitespace(w: *Writer, slice: []const u8, options: Options) Error!void {
    for (slice) |byte| switch (byte) {
        '\t' => try w.splatByteAll(' ', options.indent_delta),
        '\r' => {},
        else => try w.writeByte(byte),
    };
}

/// Check if a node tag represents a block expression.
...
/// Check if a node tag represents a control flow statement (if/for/while/switch).
...
/// Check if a node tag requires special spacing when used in slice operations.

...
const usage_fmt =
    ...
    \\  --config [file]        Path to .zfmt.zon configuration file
    \\
    \\
;
...
pub fn run(gpa: Allocator, arena: Allocator, args: []const []const u8) !void {
    ...
    var config_file = std.array_list.Managed(u8).init(gpa);
        ...
        while (i < args.len) : (i += 1) {
            ...
            if (mem.startsWith(u8, arg, "-")) {
                if (mem.eql(u8, arg, "-h") or mem.eql(u8, arg, "--help")) {
                    ...
                    if (i + 1 >= args.len) {
                        fatal("expected parameter after --config", .{});
                    }
                    i += 1;
                    const next_arg = args[i];
                    try config_file.appendSlice(next_arg);
                } else {
    ...
    const config_file_path = if (config_file.items.len > 0)
        config_file.items
    else if (try scanConfig(gpa)) |discovered_config|
        discovered_config
    else
        "";

    const should_free_config = config_file.items.len == 0 and config_file_path.len > 0;
    defer if (should_free_config) gpa.free(config_file_path);
    ...
    if (stdin_flag) {
        ...
        var out_buffer = std.Io.Writer.Allocating.init(gpa);
        defer out_buffer.deinit();
        const fixups = Render.Fixups{};
        const options = if (config_file_path.len > 0)
            try parseConfig(gpa, config_file_path)
        else
            Render.Options{};
        defer if (config_file_path.len > 0) std.zon.parse.free(gpa, options);

        try Render.renderTree(gpa, &out_buffer.writer, tree, fixups, options);

        if (check_flag) {
            const code: u8 = @intFromBool(!mem.eql(u8, out_buffer.written(), source_code));
            process.exit(code);
        }

        return fs.File.stdout().writeAll(out_buffer.written());
    }
    ...
    var fmt: Fmt = .{
        ...
        .force_zon = force_zon,
        .color = color,
        .out_buffer = std.Io.Writer.Allocating.init(gpa),
...
fn fmtPath(
    ...
    fmtPathFile(
        ...
        error.IsDir, error.AccessDenied => return fmtPathDir(fmt, file_path, check_mode, dir, sub_path, config_file),
        else => {
...
fn fmtPathDir(
    ...
    while (try dir_it.next()) |entry| {
        ...
        if (is_dir or entry.kind == .file and (mem.endsWith(u8, entry.name, ".zig") or mem.endsWith(u8, entry.name, ".zon"))) {
            ...
            if (is_dir) {
                try fmtPathDir(fmt, full_path, check_mode, dir, entry.name, config_file);
            } else {
                fmtPathFile(fmt, full_path, check_mode, dir, entry.name, config_file) catch |err| {
...
fn fmtPathFile(
    ...
    const mode: std.zig.Ast.Mode = if (fmt.force_zon) .zon else if (mem.endsWith(u8, sub_path, ".zon")) .zon else .zig;
    ...
    const fixups = Render.Fixups{};
    const options = if (config_file.len > 0)
        try parseConfig(gpa, config_file)
    else
        Render.Options{};
    defer std.zon.parse.free(gpa, options);
    try Render.renderTree(gpa, &fmt.out_buffer.writer, tree, fixups, options);
...
pub fn parseConfig(gpa: Allocator, file_name: []const u8) !Render.Options {
    const cwd = std.fs.cwd();
    const file = try cwd.openFile(file_name, .{ .mode = .read_only });

    var file_buffer: [1024]u8 = undefined;
    var file_reader: fs.File.Reader = file.reader(&file_buffer);
    const source_code: [:0]const u8 = std.zig.readSourceFileToEndAlloc(gpa, &file_reader) catch |err| {
    ...
    const parsed = try std.zon.parse.fromSlice(Render.Options, gpa, source_code, &diag, .{
        .free_on_error = true,
...
pub fn scanConfig(gpa: Allocator) !?[]const u8 {
    var path_buffer: [std.fs.max_path_bytes]u8 = undefined;

    // Get absolute path of current directory
    const current_path = try fs.cwd().realpath(".", &path_buffer);

    var current_dir_path = try gpa.dupe(u8, current_path);
    defer gpa.free(current_dir_path);

    while (true) {
        const config_path = try fs.path.join(gpa, &[_][]const u8{ current_dir_path, ".zfmt.zon" });
        defer gpa.free(config_path);

        fs.cwd().access(config_path, .{}) catch |err| switch (err) {
            error.FileNotFound => {
                const parent = fs.path.dirname(current_dir_path);
                if (parent == null or mem.eql(u8, parent.?, current_dir_path)) {
                    // Reached filesystem root, no config found
                    return null;
                }

                // Update path to parent directory
                gpa.free(current_dir_path);
                current_dir_path = try gpa.dupe(u8, parent.?);
                continue;
            },
            else => return err,
        };

        // Found config file, return the path
        return try gpa.dupe(u8, config_path);
    }
}
